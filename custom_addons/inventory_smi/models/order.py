...
class SmiOrder(models.Model):
    ...
    order_line_ids = fields.One2many(
        "smi.order.line", "order_id", string="Bahan yang Dibutuhkan"
    )
    ...
    def action_confirm(self):
        for order in self:
            if order.state != "draft":
                raise ValidationError(
                    "Hanya order berstatus Draft yang dapat dikonfirmasi."
                )
            ...
            for line in order.order_line_ids:
                ...
                self.env["smi.activity.log"]._check_and_notify_low_stock(
                    line.material_id
                )
    ...
    def action_complete(self):
        for order in self:
            if order.state != "dikonfirmasi":
                raise ValidationError(
                    "Hanya order berstatus Dikonfirmasi yang dapat diselesaikan."
                )
    ...
    def _validate_manual_picks(self, order_line):
        ...
        if abs(total_picked - order_line.jumlah_dibutuhkan) > 0.001:
            raise ValidationError(
                f"Total pengambilan manual untuk {order_line.material_id.name} "
                f"({total_picked}) harus sama dengan kebutuhan "
                f"({order_line.jumlah_dibutuhkan})."
            )

        for pick in picks:
            pick.stock_entry_id.write(
                {
                    "jumlah_tersisa": pick.stock_entry_id.jumlah_tersisa
                    - pick.jumlah_diambil
                }
            )
...
class SmiOrderLine(models.Model):
    ...
    order_id = fields.Many2one(
        "smi.order", string="Order", required=True, ondelete="cascade", index=True
    )
    material_id = fields.Many2one(
        "smi.material", string="Bahan", required=True, ondelete="restrict"
    )
    ...
    order_pick_ids = fields.One2many(
        "smi.order.pick", "order_line_id", string="Detail Pengambilan"
    )
    ...
    def _compute_is_sufficient(self):
        for line in self:
            if line.material_id:
                line.is_sufficient = (
                    line.material_id.total_stok >= line.jumlah_dibutuhkan
                )
            else:
